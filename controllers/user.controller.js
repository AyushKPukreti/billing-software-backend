...
export const editUserProfile = async (req, res) => {
  try {
    ...
    if (invoicePreferences && typeof invoicePreferences === "object") {
      ...
      if (addressBehavior !== undefined)
        user.invoicePreferences.addressBehavior = addressBehavior;
    }

    // === Custom Profile Fields Update ===
    if (customProfileFields && Array.isArray(customProfileFields)) {
      user.customProfileFields = customProfileFields
        .map((field) => ({
...
export const addClient = async (req, res) => {
  try {
    ...
    const { companyName, address, email, phone, gstNumber, panNumber, notes } =
      req.body;
...
export const editClient = async (req, res) => {
  try {
    ...
    if (email !== undefined) {
      client.email = email?.trim() ? email.trim().toLowerCase() : "";
    }

    // This allows setting phone to empty string
    if (phone !== undefined) client.phone = phone?.trim() || "";

    if (gstNumber !== undefined) client.gstNumber = gstNumber?.trim() || "";
    if (panNumber !== undefined)
      client.panNumber = panNumber?.trim()
    ...
    if (address !== undefined) {
      client.address.street = address.street?.trim() || "";
      client.address.city = address.city?.trim() || "";
      client.address.state = address.state?.trim() || "";
...
export const updateBankDetails = async (req, res) => {
  try {
    ...
    if (accountHolderName)
      user.bankDetails.accountHolderName = accountHolderName.trim();
    if (ifscCode) {
      const ifscRegex = /^[A-Z]{4}0[A-Z0-9]{6}$/;
      if (!ifscRegex.test(ifscCode.toUpperCase())) {
        return res.status(400).json({
...
export const addBankAccount = async (req, res) => {
  try {
    ...
    if (!/^\d{9,18}$/.test(accountNumber.replace(/\s/g, ""))) {
      return res
        .status(400)
        .json({ message: "Account number must be between 9-18 digits" });
    }
...
export const addCustomUnit = async (req, res) => {
  try {
    ...
    if (duplicate) {
      return res
        .status(409)
        .json({ message: `Custom unit "${trimmedName}" already exists.` });
    }
